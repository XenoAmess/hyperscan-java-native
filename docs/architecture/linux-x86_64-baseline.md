...
- `linux-arm64`：拆分为 `linux-arm64-baseline`（ARMv8.0/NEON，`-march=armv8-a`，`BUILD_SVE=OFF BUILD_SVE2=OFF FAT_RUNTIME=off`）与 `linux-arm64`（SVE2，`-march=armv9-a`，`BUILD_SVE=ON BUILD_SVE2=ON FAT_RUNTIME=off`），详见 `docs/architecture/linux-arm64-multi-variant.md`。

[English](linux-x86_64-baseline.en.md) | 中文
...
> 注：本方案描述的 SSE4.2 baseline 构建仍是仓库的一部分，但已不再是唯一的 x86_64 产物。当前完整设计见 `docs/architecture/linux-x86_64-multi-variant.zh.md`。
...
- 不解决 `linux-arm64` 制品的兼容性问题（arm64 端已通过 `linux-arm64-baseline` / `linux-arm64` 多 variant 方案单独处理，与本方案正交）。
- 不修改 vectorscan 上游源码。
...
| `doc/architecture/linux-x86_64-baseline.zh.md` | 本文档                                          |
...
+  # Baseline-only build: SSE4.2 + POPCNT (Westmere, 2010+). 见 doc/architecture/linux-x86_64-baseline.zh.md
+  cmake -DCMAKE_BUILD_TYPE=Release \
+        -DCMAKE_INSTALL_PREFIX="$(pwd)/.." \
+        -DCMAKE_INSTALL_LIBDIR="lib" \
+        -DPCRE_SOURCE="." \
...
- `linux-arm64`：拆分为 `linux-arm64-baseline`（ARMv8.0/NEON，`-march=armv8-a`，`BUILD_SVE=OFF BUILD_SVE2=OFF FAT_RUNTIME=off`）与 `linux-arm64`（SVE2，`-march=armv9-a`，`BUILD_SVE=ON BUILD_SVE2=ON FAT_RUNTIME=off`），详见 `docs/architecture/linux-arm64-multi-variant.zh.md`。
...
- 发布配置：`jreleaser.yml`
- 本文档：`doc/architecture/linux-x86_64-baseline.zh.md`
